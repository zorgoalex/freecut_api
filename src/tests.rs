...
async fn optimize_exposes_candidate_selection_telemetry() {
    ...
    assert!(
        unique >= 1,
        "expected at least one unique signature: {selection}"
    );
    ...
    for (label, weights) in weight_profiles {
        for seed in [1_u64, 2_u64] {
            if let Some(params) = json.get_mut("params").and_then(Value::as_object_mut) {
                ...
                println!(
                    "[seed_diversity_multisheet_varied] profile={label} seed={seed} time_limit_ms={tl} restarts={restarts}"
                );
            }
            let body = serde_json::to_string(&json).unwrap();
            let (status, resp) = post_json(&app, "/v1/optimize", &body).await;
            ...
            let waste_mean = selection
                ...
                .unwrap_or(0.0);
            println!(
                "[seed_diversity_multisheet_varied] profile={label} seed={seed} unique_signatures={unique} waste_mean_mm2={waste_mean:.3}"
            );
        }
    }
}
...
#[tokio::test]
async fn optimize_invalid_fitness_weights_returns_422() {
    let app = app_for_test();
    let mut json: Value = serde_json::from_str(VALID_REQUEST).unwrap();
    if let Some(params) = json.get_mut("params").and_then(Value::as_object_mut) {
        params.insert("include_svg".to_string(), Value::Bool(false));
        params.insert(
            "fitness_weights".to_string(),
            serde_json::json!({
                "waste": -1.0,
                "compactness": 0.5
            }),
...
#[tokio::test]
async fn optimize_zero_sum_fitness_weights_returns_422() {
    let app = app_for_test();
    let mut json: Value = serde_json::from_str(VALID_REQUEST).unwrap();
    if let Some(params) = json.get_mut("params").and_then(Value::as_object_mut) {
        params.insert("include_svg".to_string(), Value::Bool(false));
...
#[tokio::test]
async fn optimize_accepts_fitness_weights() {
    let app = app_for_test();
    let mut json: Value = serde_json::from_str(VALID_REQUEST).unwrap();
    if let Some(params) = json.get_mut("params").and_then(Value::as_object_mut) {
        params.insert("include_svg".to_string(), Value::Bool(false));
