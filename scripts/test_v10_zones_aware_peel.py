"""V10 hypothesis test: zones-aware peel selection.

When two peel candidates have similar densest util (within 0.3pp),
prefer the one whose densest sheet has fewer waste regions.  This should
reduce the total waste region count across the final layout by producing
less fragmented frozen sheets.

Compares V9b baseline vs V10 on 30 seeds.
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v10")
...
print(f"\nV10 Summary (30 seeds): elapsed {elapsed:.0f}s")
...
with open(os.path.join(OUT_DIR, "v10_summary.json"), "w") as f:
