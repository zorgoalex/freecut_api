"""Quick V22 baseline: constant zone_penalty 0.8 for all peel rounds.
Run: python scripts/test_v37_v22_baseline.py
"""
...
PORT = os.environ.get("FREECUT_PORT", "8089")
...
OUT_DIR = os.environ.get("FREECUT_OUT_DIR",
    os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v37", "V22_baseline_08"))
...
def call_optimize(seed):
    ...
    partition = {"enabled": True, "peel_zone_penalties": [0.8, 0.8, 0.8]}
    if SHEET_BUDGET:
...
print(f"\nV22 baseline [0.8,0.8,0.8] Summary ({n} seeds, {elapsed:.0f}s):")
print(f"  4-sheet rate:                 {n_4}/{n}")
print(f"  Avg lead util:                {avg_lead:.2f}%")
...
summary = {
    "config": "V22_baseline_08",
    "schedule": [0.8, 0.8, 0.8],
    "n_seeds": n, "elapsed_s": round(elapsed),
    "n_4_sheet": n_4, "avg_lead_util": round(avg_lead, 2),
    "avg_min_util": round(avg_min, 2), "avg_regions": round(avg_regions, 1),
    "n_regions_le_4": n_regions_le_4, "avg_max_corner_k": round(avg_max_corner / 1e3),
}
