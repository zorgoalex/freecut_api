"""V5 hypothesis test: more_restarts as a 3rd retry strategy.
For lumpy/imbalanced failures, the first retry now doubles the
restart count (5 -> 10) instead of just perturbing the seed.
"""
...
OUT_DIR = "ai_docs/tmp/best_layouts_v5"
...
n_more_restarts = sum(1 for r in results if "more_restarts" in r["strategies"])

print(f"\nV5 Summary (30 seeds, smart retry with more_restarts):")
...
print(f"  Seeds that used more_restarts:      {n_more_restarts}/{len(results)}")

print(f"\nV4 baseline (no more_restarts, from v4_summary.json):")
print(f"  4-sheet rate:                       30/30 (100%)")
print(f"  4-sheet AND all >= 90% util:        21/30 (70.0%)")
print(f"  Avg min_util:                       90.39%")
print(f"  Avg range:                          3.63%")
print(f"  Avg max_edge_gap:                   29.7 mm")
print(f"  Avg spread:                         1.42%")
...
with open(os.path.join(OUT_DIR, "v5_summary.json"), "w") as f:
    summary = {
        ...
        "summary": {
            ...
            "n_seeds_used_more_restarts": n_more_restarts,
        },
    }
    json.dump(summary, f, indent=2)
print(f"\nSummary saved to {OUT_DIR}/v5_summary.json")
