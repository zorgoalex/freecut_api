"""V17 hypothesis test: multi-profile zones-fitness pool.

For each seed, run the same request with several per-request
`ga_override.zone_penalty` values and keep the best full-layout candidate by
geometry zones.  Requires V17 per-request GA fitness overrides in the service.
"""
...
OUT_DIR = os.environ.get(
    "FREECUT_OUT_DIR",
    os.path.join(ROOT, "ai_docs", "tmp", "best_layouts_v17_profile_pool"),
)
...
def summarize(seed, zone_penalty, data):
...
def main():
    ...
    for seed in range(1, SEEDS + 1):
        ...
        for profile in PROFILES:
            try:
                data = call_optimize(seed, profile)
                row = summarize(seed, profile, data)
                rows.append(row)
                profile_rows.append({k: v for k, v in row.items() if k != "data"})
                print(
                    f"  seed={seed:2d} zp={profile:.2f}: sheets={row['sheets']}, "
                    f"lead={row['lead_util']:5.2f}%, regions={row['n_waste_regions']}, "
                    f"corner={row['max_corner_mm2'] / 1e3:.0f}k",
                    flush=True,
                )
            except Exception as exc:
                print(f"  seed={seed:2d} zp={profile:.2f}: ERROR {exc}", flush=True)
        if not rows:
            continue
        winner = min(rows, key=pool_key)
        winners.append(winner)
        print(
            f"    -> pick zp={winner['zone_penalty']:.2f}, zones={winner['n_waste_regions']}, "
            f"lead={winner['lead_util']:.2f}%, utils={winner['utils']}",
            flush=True,
        )
    ...
    avg_min = sum(r["min_util"] for r in winners) / n
    avg_regions = sum(r["n_waste_regions"] for r in winners) / n
    avg_max_corner = sum(r["max_corner_mm2"] for r in winners) / n
    n_regions_le_4 = sum(1 for r in winners if r["n_waste_regions"] <= 4)
    n_regions_le_5 = sum(1 for r in winners if r["n_waste_regions"] <= 5)

    print(f"\nV17 Profile Pool Summary ({len(winners)} seeds): elapsed {elapsed:.0f}s", flush=True)
    print(f"  4-sheet rate:                 {n_4}/{len(winners)}", flush=True)
    ...
    print(f"  Layouts with <=4 regions:     {n_regions_le_4}/{len(winners)}", flush=True)
    print(f"  Layouts with <=5 regions:     {n_regions_le_5}/{len(winners)}", flush=True)

    ranked = sorted([r for r in winners if r["sheets"] == 4], key=pool_key)
    for i, row in enumerate(ranked[:5], start=1):
        ...
        print(
            ...
            f"zones={row['n_waste_regions']}, corner={row['max_corner_mm2'] / 1e3:.0f}k, "
            f"lead={row['lead_util']}%, utils={row['utils']}",
            flush=True,
        )

    with open(os.path.join(OUT_DIR, "v17_profile_pool_summary.json"), "w", encoding="utf-8") as f:
        json.dump(
            {
                "profiles": PROFILES,
