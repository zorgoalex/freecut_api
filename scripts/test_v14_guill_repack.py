"""V14 hypothesis test: A3 — guillotine-repack nested peel winners.

Cloned from test_v13_zones_penalty.py.  V14 adds guillotine repacking:
when a nested candidate wins the peel round, the densest sheet's pieces
are re-packed via guillotine GA.  If guillotine gives <= zones at
util >= nested - 0.5 pp, it replaces the nested result.

Compares V13 baseline vs V14 on 30 seeds.
"""
...
OUT_DIR = os.environ.get("FREECUT_OUT_DIR",
    os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v14"))
...
print(f"\nV14 Summary (30 seeds): elapsed {elapsed:.0f}s")
