"""V50: benchmark V49 visual-zone profile_pool selection on fixed seeds.
...
OUT_DIR = ROOT / "ai_docs" / "tmp" / "v50_v49_profile_pool_benchmark"
CELL_MM = 10.0
MIN_REGION_CELLS = 50
...
def write_outputs(rows: list[dict[str, Any]], responses: dict[str, dict[str, Any]], cut_gap_mm: float) -> None:
    ...
    (OUT_DIR / "v50_metrics.json").write_text(
    ...
    with (OUT_DIR / "v50_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    lines = [
        "# V50 V49 profile_pool benchmark",
        "",
    ...
    lines.append("")
    lines.append(
        "Interpretation: this is a quick branch-local smoke benchmark.  Treat it as a "
        "signal for whether V49 deserves a longer visual run, not as final quality proof."
    )
    (OUT_DIR / "v50_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
