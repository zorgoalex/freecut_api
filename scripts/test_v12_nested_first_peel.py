"""V12 hypothesis test: nested for first peel iteration only.

Nested mode is not constrained by guillotine cuts and can pack the densest
sheet more tightly.  V11 showed that alternating nested/guillotine in ALL
peel iterations gives +0.63pp lead util but +2.2 zones (nested fragments
waste).  V12 uses nested ONLY for the first (densest) peel iteration,
keeping guillotine for sheets 2-3 to minimise waste fragmentation.

Compares V10 baseline vs V12 on 30 seeds.
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v12")
...
print(f"\nV12 Summary (30 seeds): elapsed {elapsed:.0f}s")
...
with open(os.path.join(OUT_DIR, "v12_summary.json"), "w") as f:
