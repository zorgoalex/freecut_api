"""V15 hypothesis test: zones-aware GA fitness (connected components).

Replaces raw free_rects.len() fragmentation proxy with connected-component
count on free rects.  Env params: FREECUT_GA_ZONE_PENALTY (default 0.3),
FREECUT_GA_FILL_PENALTY (default 0.1).

Compares V13 baseline vs V15 on 30 seeds.
"""
...
OUT_DIR = os.environ.get("FREECUT_OUT_DIR",
    os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v15"))
...
zp = os.environ.get("FREECUT_GA_ZONE_PENALTY", "0.3")
fp = os.environ.get("FREECUT_GA_FILL_PENALTY", "0.1")
print(f"\nV15 Summary (zp={zp}, fp={fp}, 30 seeds): elapsed {elapsed:.0f}s")
...
summary_name = f"v15_zp{zp}_fp{fp}_summary.json"
