"""V11 hypothesis test: peel-generator mix (guillotine/nested alternation).

In peel iterations (non-last), even attempts use guillotine, odd attempts use
nested.  Nested is not constrained by guillotine cuts and may pack the densest
sheet more tightly, raising lead util from 94% toward 95%+.

Compares V10 baseline vs V11 on 30 seeds.
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "ai_docs", "tmp", "best_layouts_v11")
...
print(f"\nV11 Summary (30 seeds): elapsed {elapsed:.0f}s")
...
with open(os.path.join(OUT_DIR, "v11_summary.json"), "w") as f:
