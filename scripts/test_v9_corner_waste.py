"""V9 hypothesis test: corner-anchored compaction + corner_free_area tie-breaker.
...
OUT_DIR = os.path.expanduser("~/projects/freecut/spec_freecut/tmp/best_layouts_v9")
...
for seed in range(1, 31):
    ...
    print(f"  seed={seed:2d}: {mark}sheets={n_sheets}, lead={lead_util:5.2f}%, min={min_util:5.2f}%, "
          f"regions={total_regions}, max_corner={max_corner/1e3:.0f}k, attempts={results[-1]['attempts']}")
...
print(f"\nV9 Summary (30 seeds): elapsed {elapsed:.0f}s")
...
with open(os.path.join(OUT_DIR, "v9_summary.json"), "w") as f:
