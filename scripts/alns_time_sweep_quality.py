...
def make_payload(
    ...
    if alns:
        params["alns"] = {
            ...
            "temperature_start": alns_temp_start,
            "temperature_end": alns_temp_end,
            "reaction_factor": alns_reaction,
        }
...
def run_series(
    ...
    for seed in seeds:
        payload = make_payload(
            ...
            restarts=restarts,
            include_svg=False,
            alns=(endpoint == "/v1/optimize/alns"),
            alns_iterations=alns_iterations,
            alns_segment_size=alns_segment_size,
            alns_temp_start=alns_temp_start,
            alns_temp_end=alns_temp_end,
            alns_reaction=alns_reaction,
        )
...
def compare_pairwise(standard: Dict[str, Any], alns: Dict[str, Any]) -> Dict[str, Any]:
    by_seed_std = {r["seed"]: r for r in standard["records"]}
    by_seed_alns = {r["seed"]: r for r in alns["records"]}
    all_seeds = sorted(set(by_seed_std.keys()) & set(by_seed_alns.keys()))
    ...
    for seed in all_seeds:
        ...
        a_ok = a.get("status") == 200 and a.get("hard_ok", False)
        if s_ok and a_ok:
            both_ok += 1
            sk = tuple(s["sort_key"])
            ak = tuple(a["sort_key"])
    ...
    return {
        ...
        "alns_better": alns_better,
        "standard_better": std_better,
        "equal": equal,
        "alns_only_hard_ok": alns_only_ok,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep ALNS time_limit vs Standard by quality formula.")
    ...
    parser.add_argument("--alns-iterations", type=int, default=24)
    parser.add_argument("--alns-segment-size", type=int, default=6)
    parser.add_argument("--alns-temp-start", type=float, default=1.0)
    parser.add_argument("--alns-temp-end", type=float, default=0.12)
    parser.add_argument("--alns-reaction", type=float, default=0.3)
    ...
    parser.add_argument("--out", default="ai_docs/tmp/alns_time_sweep_quality.json")
    args = parser.parse_args()

    template = load_json(args.fixture)
    seeds = list(range(args.seed_start, args.seed_start + args.seeds))
    alns_limits = [int(x.strip()) for x in args.alns_time_limits.split(",") if x.strip()]
    ...
    print(f"[info] alns_time_limits={alns_limits}")
    ...
    standard = run_series(
        ...
        alns_segment_size=args.alns_segment_size,
        alns_temp_start=args.alns_temp_start,
        alns_temp_end=args.alns_temp_end,
        alns_reaction=args.alns_reaction,
    )
    print(
        "[standard] "
    ...
    for tl in alns_limits:
        ...
        cmp = compare_pairwise(standard, alns)
        ...
        better_than_std = (
            cmp["alns_better"] > cmp["standard_better"]
            and alns["hard_ok_rate"] >= standard["hard_ok_rate"]
            and alns["ok_rate"] >= standard["ok_rate"]
        )
        ...
        print(
            "[alns] "
            f"tl={tl} ok_rate={alns['ok_rate']:.3f} hard_ok_rate={alns['hard_ok_rate']:.3f} "
            f"alns_better={cmp['alns_better']} std_better={cmp['standard_better']} "
    ...
    report = {
        ...
        "first_time_limit_alns_better_than_standard_ms": first_better_than_std_tl,
    }
