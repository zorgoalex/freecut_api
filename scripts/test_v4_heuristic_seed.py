"""V4 hypothesis test: heuristic seeding (FFD via vendor's
build_guillotine_heuristic / build_nested_heuristic) injected into the
candidate pool.

The V3 service handles retries internally.  This script just sends 30
single-shot requests and reports the per-seed metrics.
"""
...
OUT_DIR = "ai_docs/tmp/best_layouts_v4"
...
print(f"\nV4 Summary (30 seeds, smart retry + heuristic seeding):")
...
print(f"\nV3 baseline (no heuristic seeding, from v3_summary.json):")
...
with open(os.path.join(OUT_DIR, "v4_summary.json"), "w") as f:
    summary = {
    ...
    json.dump(summary, f, indent=2)
print(f"\nSummary saved to {OUT_DIR}/v4_summary.json")
